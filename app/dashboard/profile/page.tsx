    ...
    const fetchProfile = async () => {
      try {
        ...
        const { data, error: userError } = await supabase
          .from("users")
          .select(
            "id, name, bio, profile_pic, gallery_images, dob, gender, religion, province, interests, badges"
          )
          .eq("id", userId)
          .maybeSingle(); // ✅ FIXED
    ...
    try {
      ...
      if (profileFile) {
        const { data, error: uploadError } = await supabase.storage
          .from("user-images")
          .upload(`users/${userId}/profile-${Date.now()}`, profileFile, {
            upsert: true,
          });
      ...
      if (galleryFiles && galleryFiles.length > 0) {
        for (let i = 0; i < galleryFiles.length; i++) {
          if (galleryUrls.length >= 7) break;
          const file = galleryFiles[i];
          const { data, error: uploadError } = await supabase.storage
            .from("user-images")
            .upload(`users/${userId}/gallery-${Date.now()}-${i}`, file, {
              upsert: true,
            });
          if (uploadError) throw uploadError;
          const { data: pub } = supabase.storage
      ...
      <div className="relative w-full max-w-md bg-white/30 backdrop-blur-lg border border-gray-300/50 p-6 rounded-3xl shadow-2xl">
        ...
        <div className="flex flex-col items-center">
          <div
            ...
            style={{
              borderImage:
                "linear-gradient(45deg, green, yellow, red, black, white) 1",
            }}
          >
            <Image
          ...
          <p className="text-gray-200 italic mt-2 break-words whitespace-pre-line">
            {userData?.bio ??
              "Tell others about yourself (min 30 characters)..."}
          </p>
        </div>

        {/* ✅ Editable Gallery */}
        <div className="mt-6">
          <h2 className="text-lg font-semibold text-green-700 mb-2 text-left">
            Gallery
          </h2>
          <div className="flex space-x-3 overflow-x-auto py-2">
            {userData?.gallery_images?.map((url, idx) => (
              <div
                key={idx}
                className="w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden shadow-md relative"
              >
                <Image
                  src={url}
                  alt={`Gallery ${idx + 1}`}
            ...
            {/* ✅ Add Image Button */}
            {isEditing &&
              (userData?.gallery_images ?? []).length < 7 && (
        ...
        <div className="mt-6 text-left">
          ...
          {!isEditing ? (
            <ul className="text-gray-100 space-y-2">
              ...
              <li>
                <strong>Religion:</strong> {userData?.religion ?? "Not set"}
              </li>
              <li>
                <strong>Province:</strong> {userData?.province ?? "Not set"}
              </li>
              <li>
          ...
          {!isEditing ? (
            <>
              <button
                ...
                className="block w-full text-center bg-gradient-to-r from-green-600 via-yellow-500 to-red-600 text-white py-3 rounded-xl font-semibold shadow-lg hover:opacity-90 transform hover:scale-105 transition-all duration-300"
              >
                Edit Profile
              </button>
              <button
                onClick={handleDeleteProfile}
                className="block w-full text-center bg-red-600 text-white py-3 rounded-xl font-semibold shadow-lg hover:bg-red-700 transform hover:scale-105 transition-all duration-300"
              >
                Delete Profile
              </button>
            </>
            ...
            <>
              <button
                ...
                className="block w-full text-center bg-green-600 text-white py-3 rounded-xl font-semibold shadow-lg hover:bg-green-700 transform hover:scale-105 transition-all duration-300"
              >
                Save Changes
              </button>
              <button
                onClick={() => setIsEditing(false)}
                className="block w-full text-center bg-gray-600 text-white py-3 rounded-xl font-semibold shadow-lg hover:bg-gray-700 transform hover:scale-105 transition-all duration-300"
              >
                Cancel
              </button>
            </>
