...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!form.bio || form.bio.trim().length < 30)
      return setError("Bio must be at least 30 characters.");
    if (form.interests.length === 0)
      return setError("Select at least 1 interest.");
    ...
    try {
      ...
      // 2️⃣ Update the placeholder profile created by the trigger
      ...
      <div className="relative bg-white/30 backdrop-blur-lg border border-gray-300/50 p-10 rounded-3xl shadow-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <label
                ...
                className={`flex items-center space-x-2 p-2 rounded-lg transition ${
                  form.interests.includes(i)
                    ? "bg-gradient-to-r from-green-600 via-yellow-400 to-red-600 text-white"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-green-600 via-yellow-400 to-red-600 text-white py-3 rounded-xl font-semibold shadow-lg hover:shadow-2xl transition-all"
          >
